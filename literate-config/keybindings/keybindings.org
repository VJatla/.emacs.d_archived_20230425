...
** Buffer selection
   #+begin_src emacs-lisp
     ...
     (ryo-modal-key "m SPC j" 'set-mark-command :then '(beginning-of-line) :name "mark-line")
     (ryo-modal-key "m SPC l" 'set-mark-command :then '(end-of-line) :name "mark-line")
   #+end_src
** Rectangle mode
#+begin_src emacs-lisp
